...
if __name__ == '__main__':
    ...
    def find_mapping(test_input: str, test_output: str) -> Dict[str,int]:
    ...
    for i in range(len(input_data)):
        code_to_num = find_mapping(input_data[i],output_data[i])
        o_vals = []
        for od in output_data[i].split(' '):
            o_vals.append(str(code_to_num[''.join(sorted(od))]))
